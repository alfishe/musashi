...
* OPCODE: 8K Page Walk
* Test 68040 3-level page table walk with 8KB pages
* Address 0x10000000:
*   Root index (bits 31-25): 0x08
*   Pointer index (bits 24-18): 0x00
*   Page index (bits 17-13): 0x00
*   Page offset (bits 12-0): 0x0000

op_WALK_8K:

    .set TEST_ADDR, 0x10000000

    * Set up DTT0/ITT0 to cover 0x00xxxxxx (code/data area)
    mov.l   #0x0000C000, %d0
...
clear_loop:
    ...
    * Page entry at index 0 -> physical page (8K aligned)
    mov.l   #0x10000003, PT_PAGE  | Physical addr + UDT=3

    * Enable MMU with 8K pages (TC bit 14 = 1)
    mov.l   #0xC000, %d0        | E=1, P=1
