...
* OPCODE: PTEST M bit
* Test PTEST detecting modified bit in page descriptor
...
clear_loop:
    ...
    * Set up page table for 0x10000000 with M=1
    mov.l   #PT_PTR, %d0
    ori.l   #0x03, %d0
    ...
    * Page descriptor with M bit set (bit 4)
    mov.l   #0x10000013, PT_PAGE  | Physical addr + M=1 + UDT=3
    ...
    * Check M bit (modified) - bit 3 in MMUSR
    andi.l  #0x08, %d0
    beq     TEST_FAIL           | M bit should be set
